...
Store expensive dependency notes at `doc/deps/<dependency>/<resolved-version>.md` unless the project explicitly declares another path.

## Before Upstream Exploration

Before broad third-party dependency exploration:

1. Determine the exact resolved version from the project's lockfile, dependency manifest, package-manager metadata, vendored source, or other authoritative resolution record.
2. Determine relevant enabled options, feature flags, build variants, platform selectors, or transitive integration settings using the project's dependency tooling when needed.
3. Consult the matching `doc/deps/<dependency>/<resolved-version>.md` note if it exists.
...
- Dependency name, exact version, enabled options, and verification date.
...
- Integration gotchas, platform constraints, enabled-option caveats, and feature-flag caveats.
...
- Enabled options or feature flags change.
...
- Exact version and enabled options or feature flags.
